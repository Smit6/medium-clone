  ...
  return (
    <main>
      ...
      <article className='max-w-3xl mx-auto p-5'>
        ...
        <h2 className='text-xl font-light text-gray-500'>{props.post.description}</h2>
        <div className='flex items-center space-x-2'>
          <img className='h-10 w-10 rounded-full' src={urlFor(props.post.author.image).url()!} alt={""} />
          <p className='font-extralight text-sm'>Blog post by <span className='text-green-600'>{props.post.author.name}</span> - Published at {new Date(props.post._createdAt).toLocaleString()}</p>
        </div>
        <div className='mt-10'>
          <PortableText
...

      {
        submitted ? (
          <div className='flex flex-col p-10 my-10 bg-yellow-500 text-white max-w-2xl mx-auto'>
            <h3 className='text-3xl font-bold'>Thanks you for submitting your comment!</h3>
          ...
          <input type='submit' className='shadow bg-yellow-500 hover:bg-yellow-400 focus:shadow-outline focus:outline-none text-white font-bold py-2 px-4 cursor-pointer'/>
        </form>
        )
      }
